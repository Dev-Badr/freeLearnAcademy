...
class ItemBase(models.Model):
	...
	def render(self):
		return render_to_string(
			'track/content/{}.html'.format(self._meta.model_name),
			{'item': self}
		)
...
class Course(TrackBase):
	...
	# 	_('صورة'), 
	# 	upload_to = 'courses/img/%Y/%m/%d',
	# 	blank=True
	# )

	joines = models.PositiveIntegerField(_("المنضمون"), default=0)
...
class Unit(TrackBase):
	...
	# image = models.ImageField(
	# 	_('صورة'), 
	# 	upload_to='courses/unit/img/%Y/%m/%d', 
	# 	blank=True
	# )

	last_lecture = models.PositiveIntegerField(blank=True, null=True)

	# order field can't take verbose_name 1st argument
	order = OrderField(
		blank=True, 
		for_fields=['course'], 
...
class Practice(TrackBase):
	...
	def get_absolute_url(self):
	  	return reverse('track:practice-detail', 
	  		kwargs = {'slug':self.slug}
  		)
...
class Track(TrackBase):
	...
	practices  = models.ManyToManyField(
		Practice, 
		related_name='track', 
		verbose_name=_("تمارين")
	)

	# image = models.ImageField(
	# 	_('صورة'), 
	# 	upload_to='tracks/img/%Y/%m/%d', 
	# 	blank=True
	# )

	class Meta:
		verbose_name = _("مسار")
		verbose_name_plural = _("مسارات")
